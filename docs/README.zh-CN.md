...
[English README](../README.md) · [真实新用户验证记录](adoption-validation.md)
...
命令验证前后代码内容与验收定义必须一致，完成时会再次计算摘要。固定排除运行状态、
证据、迁移档案、验证尝试、交接、报告、`.harness/artifacts/` 和运行时的
...
退出码：成功或就绪 `0`、验收或门禁未满足 `1`、配置/用法错误 `2`、用户中断 `130`。
